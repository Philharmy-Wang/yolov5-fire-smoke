...
def plot_labels(labels, names=(), save_dir=''):
    ...
    c, b = labels[:, 0], labels[:, 1:].transpose()  # c : classes, b : boxes
    nc = int(c.max() + 1)  # number of classes
    x = pd.DataFrame(b.transpose(), columns=['x', 'y', 'width', 'height'])

    # seaborn correlogram
    sn.pairplot(x, corner=True, diag_kind='auto', kind='hist', diag_kws=dict(bins=50), plot_kws=dict(pmax=0.9))
    plt.savefig(os.path.join(save_dir, 'labels_correlogram.jpg'), dpi=800)
    plt.close()
    ...
    plt.savefig(os.path.join(save_dir, 'labels.jpg'), dpi=800)
    matplotlib.use('Agg')
    plt.close()
...
all_files = glob(r'/home/gb/yolov5/VOCdevkit/VOC2007/labels/*.txt')
shapes = []  # 标注框
ids = []  # 类别名的索引
